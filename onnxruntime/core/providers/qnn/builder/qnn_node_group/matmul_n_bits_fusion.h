...
///                                 scales
/// <summary>                         DQ
/// Represents a fusion of a DQ - MatMulNBits - Q
...
/// </summary>
class MatMulNBitsQDQFusion : public IQnnNodeGroup {
 public:
  MatMulNBitsQDQFusion(const NodeUnit& matmul_n_bits_unit, const NodeUnit& input_dq_unit, const NodeUnit& output_q_unit, const NodeUnit& scale_dq_unit);
  ORT_DISALLOW_COPY_AND_ASSIGNMENT(MatMulNBitsQDQFusion);
  ...
  std::string_view Type() const override { return "MatMulNBitsQDQFusion"; }

  /// <summary>
  /// Traverses graph to check if the given starting NodeUnit is part of a valid sequence.
  /// If so, returns a IQnnNodeGroup that contains the MatMulNBits, 2x DQ and Q NodeUnits.
  /// </summary>
